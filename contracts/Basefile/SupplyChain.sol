...
contract SupplyChain is Ownable,ManufacturerRole,WholesalerRole,RetailerRole{
  ...
  struct Item {
    ...
    address manufacturerID;         // Metamask-Ethereum address of the Manufacturer // ADDED PAYABLE
...
event ReceivedByRetailer(uint upc);            //11
  ...
  modifier verifyCaller (address _address) {
    require(msg.sender == _address, "not the address of owner.");
    _;
  }
...
//Item State Modifiers
  modifier producedByManufacturer(uint _upc) {
    require(items[_upc].itemState == State.ProduceByManufacturer, "This function can only be performed in State 'producedByManufacturer'.");
    _;
  }

  modifier processByManufacturer(uint _upc) {
    require(items[_upc].itemState == State.ProcessedByManufacturer, "This function can only be performed in State 'processByManufacturer'.");
    _;
  }

  modifier packagedByManufacturer(uint _upc) {
    require(items[_upc].itemState == State.PackageByManufacturer, "This function can only be performed in State 'packagedByManufacturer'.");
    _;
  }

  modifier forSaleByManufacturer(uint _upc) {
    require(items[_upc].itemState == State.ForSaleByManufacturer, "This function can only be performed in State 'forSaleByManufacturer'.");
    _;
  }

  modifier purchasedByWholesaler(uint _upc) {
    require(items[_upc].itemState == State.PurchasedByWholesaler, "This function can only be performed in State 'purchasedByWholesaler'.");
    _;
  }

  modifier shippedByManufacturer(uint _upc) {
    require(items[_upc].itemState == State.ShippedByManufacturer, "This function can only be performed in State 'shippedByManufacturer'.");
    _;
  }

  modifier receivedByWholesaler(uint _upc) {
    require(items[_upc].itemState == State.ReceivedByWholesaler, "This function can only be performed in State 'receivedByWholesaler'.");
    _;
  }

  modifier forSaleByWholesaler(uint _upc) {
    require(items[_upc].itemState == State.ForSaleByWholesaler, "This function can only be performed in State 'forSaleByWholesaler'.");
    _;
  }

  modifier shippedByWholesaler(uint _upc) {
    require(items[_upc].itemState == State.ShippedByWholesaler, "This function can only be performed in State 'shippedByWholesaler'.");
    _;
  }

  modifier purchasedByRetailer(uint _upc) {
    require(items[_upc].itemState == State.PurchasedByRetailer, "This function can only be performed in State 'purchasedByRetailer'.");
    _;
  }

  modifier receivedByRetailer(uint _upc) {
    require(items[_upc].itemState == State.ReceivedByRetailer, "This function can only be performed in State 'receivedByRetailer'.");
    _;
  }
...
*/
  function produceItemByManufacturer(uint _upc, string memory _productNotes, uint _price) public
    ...
    newProduce.manufacturerID = msg.sender; // Metamask-Ethereum address of the Manufacturer
...
*/
  function purchaseItemByWholesaler(uint _upc) public payable
    ...
    {
    address payable ownerAddressPayable = _make_payable(items[_upc].manufacturerID); // make ManufacturerID payable
  ...
  */
  function shippedItemByManufacturer(uint _upc) public payable
    ...
    verifyCaller(items[_upc].manufacturerID) // check msg.sender is manufacturerID
    {
